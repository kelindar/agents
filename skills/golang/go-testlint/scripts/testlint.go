	...
	err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		...
		if len(fileTests) >= 5 && subtests == 0 {
			manyFuncs = append(manyFuncs, fileIssue{
				path:   rel,
				reason: fmt.Sprintf("%d top-level Test funcs, 0 t.Run subtests ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â consider a table test", len(fileTests)),
			})
		}
		return nil
	})
